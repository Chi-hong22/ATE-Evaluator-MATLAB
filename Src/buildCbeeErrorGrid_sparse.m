function [value_grid, overlap_mask, grid_meta, map_grid] = buildCbeeErrorGrid_sparse(measurements, gridParams)
    % BUILDCBEEERRORGRID_SPARSE  稀疏 CBEE 栅格构建 (压缩结构数组 + 可选 KD-Tree)
    %
    % 【定位】
    %   这是对原稠密版本 buildCbeeErrorGrid 的内存友好替代方案。当前版本不是最初的
            ...
            for sid_tmp = 1:num_submaps
                ...
                if ~isempty(pts_all) && size(pts_all,1) >= kdtree_min_points
                    ...
                    try
                        ...
                        warning('buildCbeeErrorGrid_sparse:KDTreeBuildFailed','KDTreeSearcher 构建失败: %s (回退 bruteforce)', ME.message);
                        kdtree_structs = [];
    ...
    if ~strcmpi(elevation_interp,'none')
        ...
        if any(known(:)) && any(~known(:))
            try
                ...
                warning('buildCbeeErrorGrid_sparse_compact:ElevationInterpFailed','%s (保持原 NaN)', ME.message);
            end
        end
    end
